...
test_endpoint_addition <- function(endpoint_function, endpoint_name, expected_path) {
  test_that(cli::format_inline("{endpoint_name} adds endpoint to router and returns modified router"), {
    ...
    expect_true(
      ...
      info = cli::format_inline("Expected path {.path {expected_path}} not found in routes")
    )
